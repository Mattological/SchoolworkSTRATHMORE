#is.integer(4)#integers in R are specified using L eg. class(4L)='integer'
even
append(evenYo,1)
#question1
addToList<-function(myL){
#input should be a numeric vector
newEvenList<-c() 
newOddList<-c()
for(i in myL){if(myL[i]%%2==0){
y<-myL[i]
newEvenList<-(newEvenList,y)}
else
{x<-myL[i]
newOddList<-c(newOddList)}
}
print(newEvenList,newOddList)
}
addToList(xOdd)   
